...
from .models import Asset, AssetFolder, AssetMetadata, Version, Tag, AssetTag, MetadataField,Folder
...
def upload_asset_view(request):
    ...
    with transaction.atomic():
        ...
        if new_folder_name:
            # Create new folder under media
            new_folder = Folder.objects.create(
                name=new_folder_name.strip(),
                parent_folder=root_media,
                created_by=request.user,
            )
            AssetFolder.objects.create(asset=asset, folder=new_folder)
        elif folder_id:
            # Use selected folder
            try:
                selected = Folder.objects.get(folder_id=folder_id)
                AssetFolder.objects.create(asset=asset, folder=selected)
            except Folder.DoesNotExist:
                return JsonResponse({"detail": "Folder not found."}, status=404)
        else:
            # --ROOT-- → assign to "media"
            AssetFolder.objects.create(asset=asset, folder=root_media)
