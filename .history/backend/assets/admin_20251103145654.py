...
class AssetAdmin(admin.ModelAdmin):
    list_display = ('asset_id', 'name', 'asset_type', 'display_folders', 'uploaded_by', 'uploaded_at')
    list_filter = ('asset_type', 'uploaded_by')
    search_fields = ('name',)

    def display_folders(self, obj):
        """Show all folders the asset belongs to (comma-separated)."""
        folders = obj.folder_mappings.values_list('folder__name', flat=True)
