...
export default function AssetLoader({ view, searchQuery = "" }: AssetLoaderProps) {
  ...
  // Parse query string produced by FilterMenu
  const parseParams = (q: string) => {
    ...
    const sort = (p.get("sort") || "date").toLowerCase();

    return { categories, tags, start, end, sort };
  };
  ...
  useEffect(() => {
    ...
    let result = allAssets.filter((a) => {
      ...
      const paramCats = (filterParams.categories ?? filterParams.category ?? [])
        .map(String)
        .map((s) => s.toLowerCase())
        .filter(Boolean);
    ...
    // Apply sorting (default: date newest -> oldest)
    const sortKey = (filterParams.sort || "date").toLowerCase();
    try {
      if (sortKey === "name") {
      ...
      // ignore sort errors
      console.warn("[AssetLoader] sort error", e);
    }
