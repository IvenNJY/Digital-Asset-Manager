...
export default function AddAssetsToFolderDialog({ folderId, open, onOpenChange, onAssigned }: Props) {
    ...
    try {
        ...
        Array.from(selected).map((id) =>
            fetch(`${BackendRoute}/api/assets/folders/assign/`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
            body: JSON.stringify({ asset_id: Number(id), folder_id: folderId }),
            }).then((res) => {
            if (!res.ok) return res.json().then((e) => Promise.reject(new Error(e.detail || "Assign failed")));
            })
        )
        );
        toaster.create({ title: "Assets added to folder", type: "success" });
        onOpenChange(false);
        setSelected(new Set());
        onAssigned?.();
    } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : String(e);
        toaster.create({ title: "Failed to add assets", description: msg, type: "error" });
    }
    };
    ...
    <Dialog.Root 
        ...
        <Dialog.Positioner>
            <Dialog.Content>
            <Dialog.Header>
                <Dialog.Title>Add assets to folder</Dialog.Title>
            </Dialog.Header>
            <Dialog.Body>
                {loading ? (
                <Spinner />
                ...
                <>
                    ...
                    {candidates.length === 0 ? (
                    <Text fontSize="sm">No assets to add.</Text>
                    ) : (
                                                            <CheckboxGroup
                                                                name="assets"
                                                                value={[...selected]}
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            <HStack justify="space-between" w="full">
                                                                ...
                                                                <Text fontSize="xs" opacity={0.7}>{a.asset_type}</Text>
                                                            </HStack>
                                                        </Checkbox.Root>
                                                    )
                                                })}
